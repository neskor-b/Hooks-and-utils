...
export type ReactMediaRecorderHookProps = {
  ...
  screen?: boolean;
  /**
   * When true, captures system audio from the tab/screen being shared
   * instead of microphone audio. Only works when screen=true.
   */
...
export function useReactMediaRecorder({
  ...
  const getMediaStream = useCallback(async () => {
    ...
    try {
      if (customMediaStream) {
        ...
        // Add microphone audio if requested (and not using system audio only)
  ...
  }, [audio, video, screen, systemAudio]);
  ...
  const stopRecording = () => {
    if (mediaRecorder.current) {
      if (mediaRecorder.current.state !== 'inactive') {
        ...
        // Note: chunks are cleared in onRecordingStop after blob is created
      }
    }
  };
